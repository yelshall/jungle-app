...
io.on('connection', (socket) => {
	...
	socket.on("getRecommendations", async (request, callback) => {
		const event_factors = await recommendEvents.recommendEvents(request.sid);

		let ids = [];
		for (let i = 0; i < event_factors.length; i++) {
			ids[i] = event_factors[i].event_id;
		}
