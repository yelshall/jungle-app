...
var updateStudent = (sid, update, callback) => {
	if (update.type === "CHANGE_FIELD") {
		switch (update.field) {
			case "NAME":
				updateStudentName(sid, update.firstName, update.lastName, callback);
				break;
			case "EMAIL":
			...
			case "EXPO_TOKEN":
				...
				break;
			case "PASSWORD":
				updateStudentPassword(sid, update.password, callback);
				break;
			default:
		...
		switch (update.field) {
			case "INTERESTED_EVENT":
				...
				break;
			case "CONFIRMED_EVENT":
				removeConfirmedEvent(sid, update.eid, callback);
				break;
			case "RECOMMENDED_EVENT":
...
var updateStudentName = (sid, firstName, lastName, callback) => {
	schemas.Student.findByIdAndUpdate(sid,
		{ $set: { 'fullName.firstName': firstName, 'fullName.lastName': lastName } },
		callback);
};
...
var updateStudentPassword = (sid, password, callback) => {
	let salt = bcrypt.genSaltSync(10);
