	...
	let onPress = () => {
		if (!isEditing) {
			...
			if(event.eventName !== event_NME) {
				socket.emit('updateEventHost', {uid: loginState.id, eid: event._id,  update: {newName: event_NME, field: "EVENT_NAME", type: "CHANGE_FIELD"}}, (err, res) => {

				})
			}

			if(event.location !== event_loc) {
				socket.emit('updateEventHost', {uid: loginState.id, eid: event._id,  update: {location: event_loc, field: "EVENT_LOCATION", type: "CHANGE_FIELD"}}, (err, res) => {
					
				})
			}

			if(event.descripton !== event_des) {
				socket.emit('updateEventHost', {uid: loginState.id, eid: event._id,  update: {description: event_des, field: "EVENT_DESCRIPTION", type: "CHANGE_FIELD"}}, (err, res) => {
					
				})
			}

			if(event.dateTime !== event_time) {
				socket.emit('updateEventHost', { uid: loginState.id, eid: event._id,  update: {dateTime: event_time, field: "EVENT_DATETIME", type: "CHANGE_FIELD"}}, (err, res) => {
					
				})
			}

			if(event.endDateTime !== end_event_time) {
				socket.emit('updateEventHost', {uid: loginState.id, eid: event._id,  update: {endDateTime: end_event_time, field: "EVENT_ENDDATETIME", type: "CHANGE_FIELD"}}, (err, res) => {
					
				})
			}

			if(event.maxStudents !== maxStudents) {
				socket.emit('updateEventHost', {uid: loginState.id, eid: event._id, update: {maxStudents: maxStudents, field: "EVENT_MAXSTUDENTS", type: "CHANGE_FIELD"}}, (err, res) => {
				})
			}
			setTextValue("Edit");
			setEdit(false);
