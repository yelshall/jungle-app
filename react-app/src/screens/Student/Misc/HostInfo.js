...
export default function Host_info({ navigation, route }) {
  ...
  let onChat = () => {
    socket.emit(
      "getMessages",
      { sid: loginState.id, hid: host.current._id },
      (err, res) => {
        if (err) {
          return;
        }
    ...
    >
      {isLoading ? (
        ...
        <>
          ...
          >
            {host.current.imageURL.length == "" && (
              <>
                ...
                >
                  {host.current.hostName.charAt(0).toUpperCase()}
                </Text>
              </>
            )}
          </ImageBackground>
          ...
          <Text style={styles.description}>{host.current.description}</Text>
          ...
          {host.current.tags.length > 0 && (
            <Tags initialTags={tags} readonly={true} deleteTagOnPress={false} />
          )}
          <View
            style={{
              width: "100%",
